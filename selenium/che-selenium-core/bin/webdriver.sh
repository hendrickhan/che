...
prepareTestUserForMultiuserChe() {
    ...
    if [[ "${PRODUCT_HOST}" == "$(detectDockerInterfaceIp)" ]] || [[ "${CHE_SELENIUM_INFRASTRUCTURE}" == "openshif" ]]; then
        local time=$(date +%s)
        ...
        if [[ "${CHE_SELENIUM_INFRASTRUCTURE}" == "openshif" ]]; then
            local keycloakHost=127.0.0.1
            local kc_container_id=$(docker ps | grep keycloak_keycloak-1 | cut -d ' ' -f1)
        else
            local keycloakHost=localhost
            local kc_container_id=$(docker ps | grep che_keycloak_1 | cut -d ' ' -f1)
        fi

        local cli_auth="--no-config --server http://${keycloakHost}:8080/auth --user ${CHE_ADMIN_NAME} --password ${CHE_ADMIN_PASSWORD} --realm master"
        ...
        if [[ "$response" =~ "Created new user with id" ]]; then
           ...
           echo "[WARN] set test user credentials to be equal to admin ones in case of problem with creation of user"
